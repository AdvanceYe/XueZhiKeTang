...
-(void)addNavigationItemBtn
{
    ...
    [_rightBtn setImage:[UIImage imageNamed:@"whiteStar.png"] forState:UIControlStateNormal];
    ...
    if (_isCollected) {
        [_rightBtn setImage:[UIImage imageNamed:@"yellowStar.png"] forState:UIControlStateNormal];
    }
    else
    {
        [_rightBtn setImage:[UIImage imageNamed:@"whiteStar.png"] forState:UIControlStateNormal];
...
#pragma mark - 这里可能要更改(外观)
    ...
    _player.shouldAutoplay = isAutoPlay;
    NSLog(@"configPlayer");
...
-(void)loadData
{
    ...
    [manager GET:_courseUrl parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        else if ([jsonData isKindOfClass:[NSDictionary class]])
        {
            ...
            //传参给cacheVC
            _cacheVC.courseId = _courseModel.d_id;
            _cacheVC.courseName = _courseModel.name;
            NSLog(@"上一页:courseId:%@,courseName:%@",_cacheVC.courseId,_cacheVC.courseName);
    ...
    [manager GET:_lessonUrl parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        else if ([jsonData isKindOfClass:[NSDictionary class]])
        {
            ...
            NSArray *array = [[[jsonData objectForKey:@"result"] objectForKey:@"data"] objectForKey:@"courses"];
            //增加了lesson的一个index
...
//            NSLog(@"lesson1.name = %@",[_lessonArray[0] name]);
            ...
            _cacheVC.lessonArray = _lessonArray;
            [self configPlayer];
        }
        else
        {
