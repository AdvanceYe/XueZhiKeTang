...
@implementation CoreDataDownloadingManager

+(id)manager
{
    static CoreDataDownloadingManager *_m = nil;
    if (!_m) {
        _m = [[CoreDataDownloadingManager alloc]init];
    }
    return _m;
}
...
-(void)openDB
{
    ...
    //指定本地持久化数据的数据库
    NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/coredataDownloading.sqlite"];
    NSError *err = nil;
...
-(NSNumber *)returnLessonTotalSizeByVid:(NSString *)vid
{
    NSLog(@"%@",NSHomeDirectory());
    NSFetchRequest *req = [NSFetchRequest fetchRequestWithEntityName:@"LessonDownload"];
    req.predicate = [NSPredicate predicateWithFormat:@"vid==%@",vid];
